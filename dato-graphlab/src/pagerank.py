...
data = {'small': small, 'medium': medium, 'large': large, 'stanford': stanford, 'google': google}
...
for d in data.keys():
    ...
    pr = gl.graphlab.pagerank.create(g, max_iterations=100, threshold=0.01)
    ...
    params = config.getOutputFolder() + d + '.info'

    with open(params, 'w') as f:
        f.write("training_time = " + str(pr.training_time) + '\n')
        f.write("num_iterations = " + str(pr.num_iterations) + '\n')
        f.close()
